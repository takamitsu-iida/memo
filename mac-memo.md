...
# ファンクションキーを標準のファンクションキーとして使う

Visual Studio Codeではファンクションキーを多用するので、標準のファンクションキーの動作に戻す。

システム環境設定　→　キーボード　→　キーボードショートカットのボタン

別ウィンドウが立ち上がるので、左メニューからファンクションキーを選択。

F1、F2などのキーを標準のファンクションキーとして使用　をONに
